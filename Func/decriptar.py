def mainDecriptar():  
  ...
  with open('./bruno.txt', 'r') as arquivo:
    msg = arquivo.read().split(" ")

    msg_decriptada = []

    for i in range(0,len(msg)):
      msg_decriptada.append(pow(int(msg[i]), inverso(e, phi), n))
  ...
  with open("mensagem_desencriptada.txt", 'w') as arquivo:
    arquivo.write(str(final)) 
  print (f"\n{final}\n")
